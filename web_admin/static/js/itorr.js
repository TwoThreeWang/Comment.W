var iTorr=function(W,D,_b,$,$$){
	...
	$.x=function(d){
		return function(){
			var
			method,url,data,header,func,err,x,j,
			par,
			arg=toArr(arguments);
			...
			if(arg[0]&&typeof arg[0]=='string'&&(arg[0].match(/Authorization/igm)||arg[0].match(/Type/igm))){
				header = JSON.parse(arg.shift());
			}
			...
			// !data||x.setRequestHeader('Content-Type','application/json');

			for(var i in header){
				x.setRequestHeader(i,header[i]);
			};
